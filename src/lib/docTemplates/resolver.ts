...
const PLACEHOLDERS: Record<string, Resolver> = {
  ...
  emdMode:            (c) => v(c.analysis?.emd_details?.mode),
  emdExemption:       (c) => c.analysis?.emd_details?.msme_exemption === true ? 'MSME Exempted' : c.analysis?.emd_details?.msme_exemption === false ? 'Not Exempted' : BLANK,
  preBidMeeting:      (c) => v(c.analysis?.timeline_and_milestones?.pre_bid_meeting),
  clarificationDeadline: (c) => v(c.analysis?.timeline_and_milestones?.clarification_deadline),
