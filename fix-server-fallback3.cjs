...
const oldCode = `         // Fallback to older model if quota is hit to avoid completely blocking the user
         if (options.model === "gemini-2.5-flash") {
             ...
             modelChanged = true;
             console.warn(\`[AI Engine] Falling back to gemini-1.5-pro due to quota/rate limit.\`);
         } else if (options.model === "gemini-2.5-pro") {
             options.model = "gemini-1.5-pro";
             modelChanged = true;
             console.warn(\`[AI Engine] Falling back to gemini-1.5-pro due to quota/rate limit.\`);
         } else {
...
const newCode = `         // Fallback to older model if quota is hit to avoid completely blocking the user
         if (options.model === "gemini-2.5-flash") {
             options.model = "gemini-2.0-flash";
             modelChanged = true;
             console.warn(\`[AI Engine] Falling back to gemini-2.0-flash due to quota/rate limit.\`);
